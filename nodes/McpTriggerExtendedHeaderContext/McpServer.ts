...
export class McpServer {
	...
	async handlePostMessage(
		...
		if (toolCallInfo) {
			...
			if (tool) {
				toolCallInfo = {
					...toolCallInfo,
					arguments: mergeMcpHeaderArguments(toolCallInfo.arguments, req.headers, tool),
				};
			}
		...
		if (isListToolsRequest) {
			...
			if (transport && transport.transportType === 'sse' && messageId) {
				...
				const toolsList = tools.map((tool) => ({
					...
					inputSchema: getClientFacingInputSchema(tool),
				}));
	...
	private setupHandlers(server: Server): void {
		...
		server.setRequestHandler(CallToolRequestSchema, async (request, extra) => {
			...
			const toolArguments = mergeMcpHeaderArguments(
				rawToolArguments,
